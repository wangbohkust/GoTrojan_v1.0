...
class LevelOne1: SKScene, SKPhysicsContactDelegate{
   ...
   func didBeginContact(contact:SKPhysicsContact) {
        ...
        if collision == ColliderType.Hero.rawValue | ColliderType.Tool.rawValue {
            ...
            var foot:Int = Int(hero.position.y - hero.size.height/2)
            var up: Int!
            if hero.position == contact.bodyA.node?.position {
                up = Int(contact.bodyB.node!.position.y + 60)
            }else {
                up = Int(contact.bodyA.node!.position.y + 60)

            }
            if foot >= up - 40 && foot <= up + 40 {
                hero.physicsBody!.velocity = CGVector(dx: 0, dy: 0)
                let pinPoint = CGPoint(x:hero.position.x, y:hero.position.y+60)
